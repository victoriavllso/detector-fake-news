...
def predict(texto, model, tokenizer, device):
    ...
    with torch.no_grad():
        ...
        confianca = probs[0, pred_id].item() * 100
    ...
    label = ID2LABEL.get(pred_id, "DESCONHECIDO")
    return label, confianca, probs.squeeze().tolist()
...
def main():
    ...
    resultado, score, probs = predict(conteudo, model, tokenizer, device)

    cores = {
        "VERDADEIRA": "\033[92m",      # verde
        "FAKE": "\033[91m",            # vermelho
        "SENSACIONALISTA": "\033[93m"  # amarelo
    }
    reset = "\033[0m"
    cor = cores.get(resultado, "")
